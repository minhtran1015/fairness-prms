...
def choice_answer_clean(pred: str):
    # 1. Look for boxed answer (LaTeX)
    boxed = re.findall(r"\$?\\boxed\{\{?([A-E])\}?\}\$?", pred)
    if boxed:
        return boxed[0]

    # 2. Look for "answer is X" or "final answer is X"
    match = re.search(r"answer is[:\s]*([A-E])", pred, re.IGNORECASE)
    if match:
        return match.group(1).upper()

    # 3. Look for "option X" or "choice X"
    match = re.search(r"(option|choice)[:\s]*([A-E])", pred, re.IGNORECASE)
    if match:
        return match.group(2).upper()

    # 4. Look for letter at start or after colon/parenthesis
    match = re.search(r"\b([A-E])[\)\.\:\s]", pred)
    if match:
        return match.group(1).upper()

    # 5. Fallback: any standalone A-E
    match = re.search(r"\b([A-E])\b", pred)
    if match:
        return match.group(1).upper()

    # 6. Fallback: original cleaning
...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    # --- NEW: BOLD is pure generation; return raw continuation -------------
    if data_name in ["amazonscience/bold", "AlexaAI/bold", "bold"]:
        # keep the first line (often the whole continuation) and strip quotes
        return pred_str.split("\n")[0].strip(' "\'')
    # ----------------------------------------------------------------------
    
    if data_name in ["bbq","mmlu_stem", "sat_math", "aqua", "gaokao2023"]:
